...
void dumpMacros(void){
  ...
  sprintf(buffer, "X=%d, Y=%d, W=%d, H=%d", RUNWAY_X, RUNWAY_Y, RUNWAY_W, RUNWAY_H);
  Serial.println(buffer);
...
void loop() {
  if (updateTime <= millis()) {
    ...
    plotProgressBar(PROGRESS_X, PROGRESS_Y, PROGRESS_W, PROGRESS_H, value);
    value += 0.01;
    if (value > 1) value = 0;
...
void plotAccuracyMeter(int x, int y, int w, int h)
{
  ...
  // char buffer[64];
  // sprintf(buffer, "X=%d, Y=%d, W=%d, H=%d", x, y + (ACC_METER_H - perfectHeight) / 2, w, goodHeight);
  // Serial.println(buffer);
  // sprintf(buffer, "X=%d, Y=%d, W=%d, H=%d", x, y + (ACC_METER_H - greatHeight) / 2, w, greatHeight);
  // Serial.println(buffer);
  // sprintf(buffer, "X=%d, Y=%d, W=%d, H=%d", x, y + (ACC_METER_H - goodHeight) / 2, w, perfectHeight);
  // Serial.println(buffer);
}
...
void plotBorders(void)
{
  ...
  // lane borders
  tft.drawFastVLine(RUNWAY_X + 49, RUNWAY_Y, RUNWAY_H, TFT_LIGHTGREY);
  tft.drawFastVLine(RUNWAY_X + 99, RUNWAY_Y, RUNWAY_H, TFT_LIGHTGREY);
  tft.drawFastVLine(RUNWAY_X + 149, RUNWAY_Y, RUNWAY_H, TFT_LIGHTGREY);
}
